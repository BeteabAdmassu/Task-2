...
| POST | `/auth/register` | Create account | None | `username`, `password`, `password_confirm` | Redirect to role dashboard (success) or HTML partial with errors |
...
| POST | `/patient/delete-account` | Request account anonymization | Patient | `password` (re-authentication), anti-replay fields (required) | Session terminated, redirect to login |

### Anonymization
- `full_name` -> `"Deleted User"`; `phone` -> placeholder `"0000000"`; `date_of_birth` -> `1900-01-01`
- Gender, address, emergency contact, and encrypted ID fields set to null
- Username renamed to `deleted_<id>`; account deactivated; session terminated
- Assessment drafts deleted; completed assessments, visits, and reservations retain de-identified `patient_id` FK for medical recordkeeping
- Audit events retain structure with the renamed actor identifier
